...
namespace ya::graphics
{
	...
	private:
		...
		Microsoft::WRL::ComPtr<ID3D11DeviceContext> mContext; // dx11에서 직접적으로 디바이스객체 접근하지않고 이 객체를 이용하여 명령을 내린다.
		Microsoft::WRL::ComPtr<ID3D11Texture2D> mRenderTarget;// 최종적으로 그려질 텍스처(도화지)
		Microsoft::WRL::ComPtr<ID3D11RenderTargetView> mRenderTargetView;// 렌더타겟에 직접접근하지 않고 레더타겟뷰를 통해서 접근한다.
